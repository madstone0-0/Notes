            ...
            \item[Closed under vector addition] Let $\mbold{w}$ and $\mbold{q}$ be vectors of the appropriate form in
                  ...
                  \begin{align*}
                    ...
                    \mbold{w} + \mbold{q} & = wt^{2} + qt ^{2}           \\
                                          ...
                                          & = a t ^{2}                   \\
                    \\
                    \mbold{w} + \mbold{q} & = wt^{3} + qt ^{3}           \\
                                          & = \left( w + q \right)t ^{3} \\
                                          & \text{Let $w + q = a$}       \\
                                          & = a t ^{3}                   \\
                  \end{align*}
            \item[Closed under scalar multiplication] Again let $\mbold{w}$ and be a vector of the
                  appropriate form in each case:
                  \begin{align*}
                    c \mbold{w} & = c \left( w t^{1} \right)  \\
                                & = \left( c w \right) t ^{1} \\
                                & \text{Let $cw = a$}         \\
                                & =  at ^{1}                  \\
                    \\
...
\ex{}{
  ...
  \sol{
    ...
    \begin{align*}
      A = \begin{bmatrix} -3 & 6 & -1 & 1 & -7 \\ 1 & -2 & 2 & 3 & -1 \\ 2 & -4 & 5 & 8 & -4 \end{bmatrix} \sim \begin{bmatrix}
                                                                                                                  1 & -2 & 0 & -1 & 3  & 0 \\
                                                                                                                  0 & 0  & 1 & 2  & -2 & 0 \\
                                                                                                                  0 & 0  & 0 & 0  & 0  & 0
                                                                                                                \end{bmatrix}
...
\dfn{The Column Space of a Matrix}{
  The column space of an $m\times n$ matrix $A$, denoted by $\text{Col } A$, is the set of all linear combinations of
  the columns of $A$. If $A = \begin{bmatrix} \mbold{a}_1 & \ldots & \mbold{a}_n \end{bmatrix} $, then
  \[
    \text{Col }A = \text{Span}\{\mbold{a}_1, \ldots, \mbold{a}_n\}
  \]
}

Since $\text{Span } \{\mbold{a}_1, \ldots, \mbold{a}_n\} $ is a subspace, by theorem \ref{thm:1}, the next theorem
follows from the definition of $\text{Col }A$ and the fact that the columns of $A$ are in $\mathbb{R}^{m}$.
...
\ex{}{
  ...
  \sol{
    ...
    We first write $W$ as a set of linear combinations:
    \[
      W = \left\{ a \begin{bmatrix} 6 \\ 1 \\ -7 \end{bmatrix} + b \begin{bmatrix} -1 \\ 1\\  0 \end{bmatrix} : a, b \in
      \mathbb{R}\right\} = \text{Span } \left\{ \begin{bmatrix}
        6 \\
        1 \\
        -7
      ...
      \end{bmatrix}   \right\}
    \]
    Then we create a matrix $A$ with these columns:
    \[
      \begin{bmatrix}
        6  & -1 \\
        1  & 1  \\
...
\thm{}{
  The column space of an $m\times n$ matrix $A$ is all of $\mathbb{R}^{m}$ if and only if the equation $A\mbold{x} =
    \mbold{b}$ has a solution for each $\mbold{b}$ in $\mathbb{R}^{m}$
}

\section{The Contrast between Nul $A$ and Col $A$ }
...
\ex{}{
  \qs{}{
    ...
    \]
    \begin{enumerate}
      \item If the column space of $A$ is a subspace of $\mathbb{R}^{k}$, what is $k$?
      \item If the null space of $A$ is a subspace of $\mathbb{R}^{k}$, what is $k$?
    \end{enumerate}
  }

  \sol{
    \begin{enumerate}
      \item The columns if $A$ each have three entries so $\text{Col } A$ is a subspace of $\mathbb{R}^{k}$, where $k =
              3$
      \item A vector $\mbold{x}$ such that $A\mbold{x}$ is defined must have four entries, so $\text{Nul } A$ is a
            subspace of $\mathbb{R}^k$ where $k = 4$
    \end{enumerate}
  }
}

When a matrix is not square as with the example above the vectors in  $\text{Col } A$ and $\text{Nul } A $ live in
different "universes", for example no linear combination of vectors in $\mathbb{R}^{3}$ can produce a vector in
$\mathbb{R}^{4}$. When $A$ is square $\text{Nul } A$ and $\text{Col } A$ have the zero vector in common, and in special
cases can also have some nonzero vectors in common.

\ex{}{
  \qs{}{
    With the same $A$ find a nonzero vector $\text{Col }A$ and a nonzero vector in $\text{Nul }A$
  }

  \sol{

    $\text{Col } A = \begin{bmatrix} 2 \\ -2 \\ 3 \end{bmatrix} $ \\

    $\text{Nul }A$
    \begin{align*}
      A\mbold{x}                        & = \mbold{0}              \\
      \left[ A  \mid  \mbold{0} \right] & \sim  \begin{bmatrix}
                                                  1 & 0 & 9  & 0 & 0 \\
                                                  0 & 1 & -5 & 0 & 0 \\
                                                  0 & 0 & 0  & 1 & 0
                                                \end{bmatrix} \\
    \end{align*}
    Thus, if $\mbold{x}$ satisfies $A\mbold{x} = \mbold{0}$, then $x_1 = -9x_3$, $x_2 = 5x_3$, $x_4 = 0$, and $x_3$ is
    free. Assigning a nonzero value to $x_3$, like $1$, we obtain a vector in $\text{Nul }A$, $\mbold{x} = \left( -9, 5,
      1, 0\right) $
  }
}
