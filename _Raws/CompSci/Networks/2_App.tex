...
\dfn{Recursive Query}{
  ...
  responsible for making the necessary requests to the root, then the
  root server makes a request to the TLD server, then the TLD server
  makes a request to the authoritative server, and finally the
  authoritative server returns the IP address to the local DNS
  server, which then returns the IP address to the client.
}
