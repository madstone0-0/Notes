...
\ex{}{
  ...
  \sol{
    \begin{enumerate}
      ...
      \item There exists some people, If they are comedians, then they are funny.
...
\qs{}{
  ...
  \begin{enumerate}
    \item Every Ashesi student has taken some math course
    \item No one knows everyone
  \end{enumerate}
}
...
\sol{
  \begin{enumerate}
    \item
          \begin{align*}
            \mathbb{U}_{x} = \text{ all Ashesi students}        \\
            \mathbb{U}_{y} = \text{ all math courses}           \\
            T \left( x, y \right) \text{be "$x$ has taken $y$"} \\
            \\
            \forall x \exists y \,T \left( x, y \right)         \\
            \\
            \neg \forall x \exists y \,  T \left( x, y \right)  \equiv \exists x \forall  y \neg T \left( x, y
            \right) \tag*{By De Morgan's law for quantifiers}   \\
          \end{align*}
          There exists an Ashesi student who has not taken any math courses

    \item
          \begin{align*}
            \mathbb{U}_{x, y} = \text{ all people}                \\
            K \left( x, y  \right) \text{ be "$x$ knows $y$"}     \\
            \\
            \neg \exists x \forall y\, K \left( x, y \right)      \\
            \\
            \neg \left( \neg \exists x \forall y \, K \left( x,y \right)  \right)  \equiv \exists x \forall y \, K \left(
            x, y\right) \tag*{By De Morgan's Law for quantifiers} \\
            \text{There exists someone who knows everyone}
          \end{align*}
  \end{enumerate}
}
...
\sol{
  \begin{enumerate}
    \item If I remember to send you the address, then you sent me an e-mail message.
    \item If you were born in the United States, then you can be a citizen of this country.
    \item If you keep your textbook, then it will be a useful reference in your future courses.
    \item If their goalie plays well, then the Red Wings will win the Stanley Cup.
    \item If you get the job, then you had the best credentials.
    \item If there is a storm, then the beach will erode.
    \item If you log onto the server, then you have a valid password.
    \item If you do not begin your climb too late, then you will reach the summit.
  \end{enumerate}
}
...
\qs{}{
  ...
  \begin{enumerate}
    \item $\left( p \wedge q \right) \to p $
    \item $\neg p \to \left( p \to q \right) $
    \item $\left( p \wedge q \right) \to \left( p \to q \right)  $
  \end{enumerate}
}
...
\sol{
  \begin{enumerate}
    \item
          \begin{align*}
            \left( p \wedge q \right)  \to q & \equiv \neg \left( p \wedge q \right) \vee q \tag*{Conditional
            Disjunction Equivalence}                                                                                        \\
                                             & \equiv \left( \neg p \vee \neg q \right) \vee q \tag*{By First De Morgan's
            Law}                                                                                                            \\
                                             & \equiv \neg p \vee  \left( q \vee \neg q  \right) \tag*{By First Associative
            Law}                                                                                                            \\
    ...
    \item
          \begin{align*}
            \neg p \to  \left( p \to  q \right) & \equiv \neg  \left( \neg p \right) \vee \left( p \to q \right)
            \tag*{Conditional Disjunction Equivalence}                                                                   \\
                                                & \equiv p \vee \left( p \to q \right)  \tag*{By Double Negation Law}    \\
                                                & \equiv p \vee \left( \neg p \vee q \right) \tag*{Conditional
            Disjunction Equivalence}                                                                                     \\
                                                & \equiv q \vee  \left( \neg p \vee p \right) \tag*{By First Associative
            Law}                                                                                                         \\
                                                & \equiv q \vee T \tag*{By First Negation Law}                           \\
                                                & \equiv T \tag*{By First Domination Law}
          \end{align*}
    \item
          \begin{align*}
...
\sol{
  \begin{enumerate}
    \item
          \begin{align*}
            \mathbb{U}: \text{All drivers}                                                               \\
            \\
            S \left( x \right): x \text{ obeys the speed limit}                                          \\
            \exists x \neg S \left( x \right)                                                            \\
            \neg \left( \exists x \neg S \left( x \right)  \right) & \equiv \forall x S \left( x \right) \\
            \therefore \text{ All drivers obey the speed limit}
          \end{align*}
    \item
          \begin{align*}
            \mathbb{U}: \text{All Swedish movies}                                                         \\
            \\
            S \left( x \right): x \text{ is serious}                                                      \\
            \forall x S \left( x \right)                                                                  \\
            \neg  \left( \forall x S \left( x \right)  \right) & \equiv \exists x \neg S \left( x \right) \\
            \therefore \text{ Some Swedish movies are not serious}
          \end{align*}
    \item
          \begin{align*}
            \mathbb{U}: \text{All people}                                                                \\
            \\
            S \left( x \right): x \text{ can keep a secret}                                              \\
            \neg \exists x S \left( x \right) \equiv \forall x \neg S \left( x \right)                   \\
            \neg \left( \forall x \neg S \left( x \right)  \right) & \equiv \exists x S \left( x \right) \\
            \therefore \text{ Some people can keep a secret}
          \end{align*}
    \item
          \begin{align*}
            \mathbb{U}: \text{ All people in this class}                                                 \\
            \\
            A \left( x \right): x \text{ has a good attitude}                                            \\
            \exists x \neg A \left( x \right)                                                            \\
            \neg \left( \exists x \neg A \left( x \right)  \right) & \equiv \forall x A \left( x \right) \\
            \therefore \text{ Everyone in this class has a good attitude}
          \end{align*}
  \end{enumerate}
}

\qs{}{
  Let $T \left( x, y \right) $ mean that student $x$ likes cuisine $y$, where the domain of $x$ consists of all students
  at your school and the domain for $y$ consists of all cuisines. Express each one of these statements by a simple English
  sentence.
  \begin{enumerate}
    \item $\neg T \left( \text{Abdallah Hussein}, \text{Japanese} \right) $
    \item $\exists x T \left( x, \text{Korean} \right) \wedge  \forall x T \left( x, \text{Mexican} \right)  $
    \item $\exists y \left( T \left( \text{Monique Arsenault}, y \right) \wedge T \left( \text{Jay Johnson},y \right)   \right) $
    \item $\forall x\forall z\exists y \left( \left( x \neq z \right) \to \neg \left( T \left( x, y \right) \wedge T
            \left( z, y \right)   \right)   \right) $
    \item $\exists x\exists z\forall y \left( T \left( x, y \right) \leftrightarrow T \left( z, y \right)   \right) $
  \end{enumerate}
}

\sol{
  \begin{enumerate}
    \item Abdallah Hussein does not like Japanese cuisine.
    \item There exists some student who likes Korean cuisine, and all students like Mexican cuisine
    \item There exists a cuisine that both Monique Arsenault and Jay Johnson like
    \item
          For all students $x$, for all students $z$, there exists some cuisine $y$, that if $x$ and $z$ are not the
          same people, then both student $x$ and $z$ do not like that cuisine. \\

          $\therefore$ No two students like the same cuisine
    \item
          There exists some student $x$ and some student $z$, for all cuisines $y$, $x$ likes cuisine $y$ if and only if
          $z$ likes cuisine $y$. \\

          $\therefore$ There exists some students that like all the same cuisines.
  \end{enumerate}
}

\pagebreak
\qs{}{
  Use rules of inference to show that the hypotheses "If it does not rain or if it is not foggy, then the
  sailing race will be held and the lifesaving demonstration will go on," "If the sailing race is held, then the
  trophy will be awarded,"" and "The trophy was not awarded" imply the conclusion "It rained."
}

\sol{

  \noindent Let $p$ be "it rained"\\
  Let $q$ be "it is foggy"\\
  Let $r$ be "the sailing race will be held"\\
  Let $s$ be "the lifesaving demonstration will go on"\\
  Let $t$ be "the trophy will be awarded"\\

  \begin{math}
    \begin{deduction}
      \premise{(\neg p \vee \neg q) \to \left( r \wedge s \right) }
      \premise{ r \to t }
      \premise{ \neg t }
      \conclusion{ p }
    \end{deduction}
  ...
  \begin{table}[h!]
    \begin{center}
      \begin{tabular}{ | @{\makebox[3em][r]{\rownumber\space}} | c | c | }
        ...
        \hline
        \hline
        $\left( \neg p \vee \neg q \right) \to \left( r \wedge   s \right)  $ & Premise 1                     \\
        $r \to t$                                                             & Premise 2                     \\
        $\neg t$                                                              & Premise 3                     \\
        $\neg r$                                                              & By Modus Tollens on 2 and 3   \\
        $\neg \left( r \wedge s \right) $                                     & By Addition on 4              \\
        $p \wedge  q$                                                         & By Modus Tollens on 1 and 5   \\
        $p$                                                                   & By Simplification on 6        \\
        \hline
      \end{tabular}
    \end{center}
  \end{table}
...
\qs{}{
  Use rules of inference to show that the hypotheses "All movies produced by John Sayles are wonderful",
  and "John Sayles produced a movie about coal miners" imply the conclusion "There is a wonderful
...
\qs{}{
  Show that if $x$ is rational, then $3x - 1$ is rational.
}
...
\qs{}{
  Let $n$ be an integer. Show that $n$ is even if and only if $n^3 + 5$ is odd.
}
